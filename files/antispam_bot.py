...
class AntispamBot(Client):
    ...
    async def start(self):
        ...
        await self.log_admin(f'Started Antispam Bot "{me.username}"')
    ...
    async def stop(self):
        me = await self.get_me()
        await self.log_admin(f'Stopping Antispam Bot "{me.username}"')
        await super().stop()
    ...
    async def welcome_new_member(self, message):
        ...
        await asyncio.sleep(self.approve_notice_time)
        await self.remove_conversation(message)
